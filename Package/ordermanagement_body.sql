    ...
    */
    ...
    BEGIN
        ...
        BEGIN
            ...
            FOR item IN (
                SELECT
                    regexp_substr(p_selected_items, '[^,]+', 1, level) AS selected_item
                FROM
                    dual
                CONNECT BY
                    regexp_substr(p_selected_items, '[^,]+', 1, level) IS NOT NULL
            ) LOOP
    ...
    */
    ...
    BEGIN
        ...
        IF v_order_status = 'Pending' THEN
            ...
            FOR item IN (
                SELECT o.QUANTITY AS ORDER_QUANTITY, i.ITEM_NAME, inv.QUANTITY AS INVENTORY_QUANTITY, i.ITEM_ID -- Include item_id
                ...
                ELSE
                    ...
                    UPDATE INVENTORY
                    SET QUANTITY = QUANTITY - item.ORDER_QUANTITY
                    WHERE ITEM_ID = v_item_id; -- Use v_item_id
                    -- Commit the transaction
                    COMMIT;
